  ...
  <head>
	...
	<script>
		...
		function WSAlumnosConsultarBaja()
		{
			...
			{
				$.ajax({
					url        : "http://localhost:8080/AdminEscolar/webservice/WSAlumnosConsultarBaja.php",
					type       : "POST",
					cache	   : false,
					dataType   : 'json',
					crossDomain: true,
		...
		function WSMostrarAlumnosBaja()
		{
			var txtAlumnosSeleccionar = document.getElementById("txtAlumnosSeleccionar").value;
			alert ("Alumno id: " + txtAlumnosSeleccionar);
			
			if(txtAlumnosSeleccionar > 0)
			{
				$.ajax({
					url        : "http://localhost:8080/AdminEscolar/webservice/WSMostrarAlumnosBaja.php",
					type       : "POST",
					cache	   : false,
					dataType   : 'json',
					crossDomain: true,
					...
					success    : function(data) {
						...
						//Se activa el boton de eliminar.
						document.getElementById("btnBaja").disabled = false;
					},
						error : function() 
						{
...
<!-- Menu -->
  <body role="document">
	...
	<ul class="nav nav-tabs" role="tablist">
		...
		<li role="presentation" class="active"><a>Baja alumnos</a></li>
    </ul>
	</br>
	
	<div class="container">
	<div>	
		<form name="vform" id="form" enctype="multipart/form-data" method="POST" action="alumnosBaja">  
			...
			<?php
				...
				echo "<tr>						
						<td><select class='form-control' id='txtMaestrosSeleccionar' name='txtMaestrosSeleccionar' onChange='WSAlumnosConsultarBaja()'>";		
						$query = "SELECT * FROM maestros WHERE estatus = 1 ORDER BY nombre";
					...
					echo '</select></td>
						<td><select class="form-control" id="txtAlumnosSeleccionar" name="txtAlumnosSeleccionar" onChange="WSMostrarAlumnosBaja()" required disabled>';
							$query = "select * from alumnos where idMaestros='".$txtMaestrosSeleccionar."' and estatus =  1 ORDER BY nombre";
							$result = mysql_query($query,$connection)or die ("Error: ".mysql_error());
				...
				echo "<tr>
						<td><input type='text' id='txtNombreAlumno' name='txtNombreAlumno' value='".$txtNombreAlumno."' class='form-control' placeholder='Nombre alumno.' title='Ej. Alberto' size='20' maxlength='20' required readonly></td>
						<td><input type='text' class='form-control' id='txtApellidoPaterno' name='txtApellidoPaterno' value='".$txtApellidoPaterno."' size='20' maxlength='20' required placeholder='Apellido paterno.' title='Ej. Castro' readonly></td>
						<td><input type='text' class='form-control' id='txtApellidoMaterno' name='txtApellidoMaterno' value='".$txtApellidoMaterno."' size='20' maxlength='20' required placeholder='Apellido materno.' title='Ej. Valencia' readonly></td>
					</tr>";
				...
				echo "<tr>
						<td><input type='date' class='form-control' id='txtFechaNacimiento' name='txtFechaNacimiento' value='".$txtFechaNacimiento."' size='10' maxlength='10' required title='Ej. 30/09/1989' readonly></td>
						<td><input type='text' class='form-control' id='txtCurp' name='txtCurp' value='".$txtCurp."' size='18' maxlength='18' required placeholder='Curp.' title='Ej. BEML920313HCMLNS09' readonly></td>
						<td><input type='date' class='form-control' id='txtFechaIngreso' name='txtFechaIngreso' value='".$txtFechaIngreso."' size='10' maxlength='10' required title='Ej. 10/08/2016' readonly></td>
					</tr>";
				...
				echo "<tr>						
						<td><input type='text' class='form-control' id='txtCalle' name='txtCalle' value='".$txtCalle."' size='50' maxlength='50' required placeholder='Nombre de la calle.' title='Ej. Av. Vallarta' readonly></td>
						<td><input type='text' class='form-control' id='txtNumeroCasa' name='txtNumeroCasa' value='".$txtNumeroCasa."' size='10' maxlength='10' required placeholder='N&uacute;mero de casa.' title='25-A' readonly></td>
						<td><input type='text' class='form-control' id='txtColonia' name='txtColonia' value='".$txtColonia."' size='50' maxlength='60' required placeholder='Colonia.' title='Centro' readonly></td>
					</tr>							
				</table>";							
				?>
				...
				echo "<tr>
						<td><input type='text' class='form-control' id='txtCP' name='txtCP' pattern='[0-9]{5,9}' title='Ej. 12345' value='".$txtCP."' size='10' maxlength='10' required placeholder='C.P.' readonly></td>
						<td><input type='text' class='form-control' id='txtMunicipio' name='txtMunicipio' value='".$txtMunicipio."' size='45' maxlength='45' required placeholder='Municipio.' title='Ej. Lagos de Moreno' readonly></td>
						<td><input type='text' class='form-control' id='txtEstado' name='txtEstado' value='".$txtEstado."' size='45' maxlength='45' required placeholder='Estado.' title='Ej. Jalisco' readonly></td>
						<td><input type='text' class='form-control' id='txtPais' name='txtPais' value='".$txtPais."' size='45' maxlength='45' required placeholder='Pa&iacute;s.' title='Ej. M&eacute;xico' readonly></td>
					</tr>";
				...
				echo "<tr>
						<td><input type='text' class='form-control' id='txtContacto' name='txtContacto' title='Ej. Alejandra Rodriguez Valencia' value='".$txtContacto."' size='100' maxlength='100' required placeholder='Nombre de persona de contacto.' readonly></td>
						<td><input type='text' class='form-control' id='txtTelefono' name='txtTelefono' pattern='[0-9 -]{10,20}' title='Ej. 3310259000' value='".$txtTelefono."' size='20' maxlength='20' required placeholder='Tel&eacute;fono.' readonly></td>
					</tr>";
				...
				echo "<tr>
						<td><input type='text' class='form-control' id='txtPeriodoEscolar' name='txtPeriodoEscolar' title='Ej. 1 A, Matutino 2016 - 2017' value='".$txtPeriodoEscolar."' size='30' maxlength='25' required placeholder='Periodo escolar.' readonly></td>
					</tr>							
				</table>";							
				?>
				
				<br>	
				<input type="submit" id="btnBaja" name="btnBaja" class="btn btn-success btn-lg" value="Baja" disabled>
				<input type="button" name="btnCancelar" class="btn btn-success btn-lg" onClick="back()" value="Volver">	
				<?php
					if($txtAlumnosSeleccionar > 0 && $txtMaestrosSeleccionar > 0)
					{
						$query = "UPDATE alumnos SET estatus = 2 WHERE idAlumnos= '".$txtAlumnosSeleccionar."' ";
						mysql_query($query,$connection) or die("Error al modificar la informaciòn.".mysql_error());
